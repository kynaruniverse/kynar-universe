...
export default function ProductCard({ product }: { product: Product }) {
  const theme = useMemo(() => {
    if (!product?.world) return FALLBACK_THEME
    
    // Normalize string to match WORLD_CONFIG keys (e.g., 'home' -> 'Home')
    const worldKey = product.world.charAt(0).toUpperCase() + product.world.slice(1).toLowerCase()
    const worldConfig = WORLD_CONFIG[worldKey as keyof typeof WORLD_CONFIG]
