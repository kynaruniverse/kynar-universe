  ...
  return (
    <>
      <button 
        ...
        className={cn(
          "group relative flex h-10 w-10 items-center justify-center rounded-xl border transition-all active:scale-90 z-[90]",
          view !== 'closed' ? "bg-kyn-slate-900 text-white border-kyn-slate-900" : "bg-surface border-border text-kyn-slate-900"
        )}
      >
      ...
      {view !== 'closed' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-gutter">
          <div className="absolute inset-0 bg-canvas/40 backdrop-blur-md animate-in fade-in duration-300" onClick={closeAll} />

          <div className="relative w-full max-w-sm overflow-hidden rounded-[2.5rem] border border-border bg-white p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <button onClick={closeAll} className="absolute right-6 top-6 text-kyn-slate-400 hover:text-kyn-slate-900 z-10">
              <X size={20} />
            </button>
            ...
            {view === 'gateway' && (
              <div className="flex flex-col gap-4 py-4">
                <div className="text-center mb-4">
                  <h3 className="font-brand text-2xl font-bold">Access Portal</h3>
                  <p className="text-sm text-text-secondary font-ui mt-1">Select your entry method.</p>
                </div>
                <button onClick={() => setView('login')} className="w-full py-4 bg-kyn-slate-900 text-white rounded-2xl font-brand font-bold hover:bg-kyn-slate-800 transition-all">Sign In</button>
                <button onClick={() => setView('register')} className="w-full py-4 border border-border text-kyn-slate-900 rounded-2xl font-brand font-bold hover:bg-surface transition-all">Register Identity</button>
              </div>
            )}
            ...
            {view === 'login' && (
              <form action={login} className="space-y-5">
                <h3 className="font-brand text-2xl font-bold mb-2">Sign In</h3>
            ...
            {view === 'register' && (
              <form action={signup} className="space-y-5">
                <h3 className="font-brand text-2xl font-bold mb-2">Register</h3>
                <div className="space-y-4">
                  <div className="relative">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-kyn-slate-400" size={16} />
                    <input name="email" type="email" required placeholder="Email Address" className="w-full pl-12 pr-4 py-4 rounded-2xl border border-border bg-surface font-ui text-sm focus:ring-2 focus:ring-kyn-green-500/20 outline-none" />
            ...
            {view === 'user-menu' && (
              <div className="flex flex-col gap-3">
                ...
                <form action={logout} className="mt-2">
                  <button type="submit" className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-red-50 text-red-600 font-brand font-bold hover:bg-red-100 transition-colors">
...
function SubmitButton({ label }: { label: string }) {
  ...
  return (
    <button disabled={pending} type="submit" className="w-full flex items-center justify-center gap-2 py-4 bg-kyn-slate-900 text-white rounded-2xl font-brand font-bold hover:bg-kyn-slate-800 transition-all disabled:opacity-70">
...
function MenuButton({ href, icon: Icon, label, onClick }: any) {
  return (
    <Link href={href} onClick={onClick} className="flex items-center gap-4 px-6 py-4 rounded-2xl bg-surface border border-transparent hover:border-border hover:bg-white text-kyn-slate-900 font-brand font-bold transition-all">
