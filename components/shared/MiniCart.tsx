...
export const MiniCart = () => {
  ...
  const handleCheckout = () => {
    // We only take the first item for v1 (Lemon Squeezy single-product checkout)
    // Or you can map this to a specific bundle variant in v2
  ...
  return (
    <div className="fixed inset-x-4 bottom-24 z-[60] animate-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white dark:bg-kyn-slate-900 shadow-kyn-lift dark:shadow-none border border-kyn-slate-100 dark:border-kyn-slate-800 rounded-[2rem] p-6 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-kyn-slate-400 uppercase tracking-tighter">
                {cart.length} Asset{cart.length > 1 ? 's' : ''} Ready
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="space-y-3 max-h-40 overflow-y-auto no-scrollbar mb-6">
          {cart.map((item) => (
            <div key={item.id} className="flex justify-between items-center bg-kyn-canvas dark:bg-kyn-slate-800/50 p-3 rounded-2xl border border-kyn-slate-50 dark:border-kyn-slate-800">
              <div className="flex flex-col">
                <span className="text-xs font-bold text-kyn-slate-900 dark:text-white">{item.name}</span>
                <span className="text-[10px] font-medium text-kyn-green-600 uppercase tracking-widest">{item.world}</span>
              </div>
              <div className="flex items-center gap-3">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center px-2">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-kyn-slate-400">Total</span>
          ...
          <p className="text-[9px] text-center text-kyn-slate-400 font-medium">
            Instant Digital Delivery • VAT Included
          </p>
        </div>
      </div>
    </div>
