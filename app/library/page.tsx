/**
 * KYNAR UNIVERSE: User Library / Vault (v2.0)
 * Refactor: Type-safe, semantic, and optimized for Next.js 15
 */
...
export default async function LibraryPage() {
  ...
  // Fetch user's library with related product info
  ...
  // Normalize data
  const items: UserLibraryWithProduct[] = (rawItems || []).map((item) => ({
    ...item,
      ...
      <section className="pb-24">
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {items.map((item) => {
              const acquiredDate = item.acquired_at
              ...
              return (
                ...
                >
                  ...
                  <div className="p-8 flex-1 flex flex-col">
                    <div className="flex items-center gap-2 mb-4 text-[10px] font-bold uppercase tracking-widest text-kyn-slate-400">
                      <Clock size={12} className="text-kyn-green-600" />
                      <span>Index Date: {acquiredDate}</span>
                    </div>
