...
const EmptyCart = () => (
  <div className="flex min-h-[70vh] flex-col items-center justify-center px-gutter text-center animate-in fade-in duration-700">
    ...
    <h1 className="font-brand text-xl font-bold text-kyn-slate-900 uppercase tracking-widest">
      Selection Empty
    </h1>
    <p className="mt-2 font-ui text-[11px] uppercase tracking-[0.2em] text-kyn-slate-400">
      Your vault awaits new additions.
    </p>
    <Link 
      href="/store" 
...
const CartItem = ({ product, onRemove }: { product: any, onRemove: (id: string) => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-6">
        ...
        <button 
          onClick={() => { hapticFeedback('light'); onRemove(product.id); }}
...
const CartFooter = ({ totalPrice, count }: { totalPrice: number, count: number }) => (
...
export default function CartPage() {
  ...
  const totalPrice = items.reduce((sum, item) => sum + (getPriceFromId(item.price_id) ?? 0), 0);
  ...
  return (
    <div className="mx-auto max-w-2xl px-gutter py-12 pb-48 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      {/* Footer */}
      <CartFooter totalPrice={totalPrice} count={count} />
    </div>
  );
}