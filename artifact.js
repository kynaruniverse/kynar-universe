    ...
    content.innerHTML = `
        <div class="animate-fade-in p-2">
            ...
            <div class="grid grid-cols-2 gap-4 mb-8">
                ...
                <div class="p-3 bg-white/5 rounded-lg border border-white/5">
                    <p class="text-[10px] uppercase text-gray-500 tracking-widest">Impact</p>
                    <p class="text-sm font-bold text-emerald-400">${artifactData.impact}</p>
                </div>
            </div>

            <button onclick="initAcquisition()" class="w-full py-4 bg-white text-black font-bold rounded-xl active:scale-95 transition-transform">
                ACQUIRE ARTIFACT
            </button>
            
            <button onclick="renderMarketplace()" class="w-full mt-4 py-2 text-gray-500 text-sm underline">Return to World</button>
        </div>
    `;
};

window.initAcquisition = () => {
    if (window.navigator.vibrate) window.navigator.vibrate([30, 50, 30]);
    alert("Connection to Kynar Secure Checkout established.");
};
